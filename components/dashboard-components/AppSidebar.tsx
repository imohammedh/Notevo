                ...
                <DropdownMenuTrigger asChild className=" py-8">
                    {
                      ...
                      <Button variant="SidebarMenuButton" className=" border-none w-full flex items-center justify-between" disabled={loading}>
                        <Avatar className="rounded-lg max-w-10 max-h-10 flex items-center justify-center border border-solid border-brand_tertiary/20">
                          <AvatarImage src={User?.image} alt={User ? User.name?.charAt(0) : `${<LoadingAnimation/>}`} />
                          <AvatarFallback>{User ? User.name?.charAt(0) : <LoadingAnimation/>}</AvatarFallback>
                        </Avatar>
                        <p className=" flex flex-col items-start justify-center">
                          <span>
                            {User?.name ? `${User.name.split(" ")[0].length > 10 ? `${User.name.split(" ")[0].substring(0, 10)}...` : User.name.split(" ")[0]}${User.name.split(" ")[1] ? ` ${User.name.split(" ")[1].charAt(0)}.` : '.'}` : <LoadingAnimation className=" h-4 w-4"/>}
                          </span>
                          <span>
